...
// RADHCHAN 25-AUG-21 Enh 33273525 - Search Bug Comments Enhancement 
// ALALITHA 19-OCT-21 Bug 33458190 - ORA-20001: You have exceeded the maximum number of web service requests per workspace. Please contact your administrator  
// KKOUSHIK 30-JAN-23 Enh 34773107 - Added function to resize bug comments iFrame dynamically
////////////////////////////////////////////////////////////////////////////////////////////////////
...
function ValidateId(n, list_uid_needed, is_product_changed = 'N')
{
    Trim(n);
    if (list_uid_needed != top.lists.loaded_list  ||  !top.lists.data_done_loading)
    {
		if (n == top.bugframe.status_field ) {
			...
			/*Changes to get status list in Create Bug Screen RSVL2*/
			if (screen_type !== 'C') {
                        
			try {
				var p_rptno = top.bugframe.document.getElementById("rptno").value; //AVIMALA - RSVL2
			}
			catch (e) {
				console.log('No RPTNO');
			}
			}
			/*End Changes to get status list in Create Bug Screen RSVL2*/
			if (screen_type !== 'C' && is_product_changed == 'N') {
			$.ajax({url:document.location.protocol + "//" + document.location.host
								   + "/pls/bug/webbug_status.get_wf_status_desc?p_rptno=" + p_rptno + "&p_screen_type=" + screen_type +"&p_gp_val=" + gp_value + "&p_status_code=" + status , success: function(result){
...
function ClearProbTypeFields()
{

	if (top.bugframe.document.bugform.elements[top.bugframe.prob_type_field] != null)
    {
        top.bugframe.document.bugform.elements[top.bugframe.prob_type_field].value = '';
        top.bugframe.document.bugform.elements[top.bugframe.prob_type_field+1].value = '';
        top.bugframe.document.hiddenform.elements[top.bugframe.prob_type_field].value = 'Y';
    }
...
function SetStatusListWflow(n, workflow_id, gp_value, screen_type) {
   ...
   if (screen_type) {
      self.lists.location.href = document.location.protocol + "//" + document.location.host
                               + "/pls/bug/webbug_status.get_status_list_p_html?p_workflow_id=" + workflow_id + "&p_gp_val=" + gp_value + "&p_screen_type=" + screen_type;
   } else {
      self.lists.location.href = document.location.protocol + "//" + document.location.host
                               + "/pls/bug/webbug_status.get_status_list_p_html?p_workflow_id=" + workflow_id + "&p_gp_val=" + gp_value;
...
{
   suffix = top.bugframe.document.bugform.elements[top.bugframe.product_id_field].value + '_' + top.bugframe.document.bugform.elements[top.bugframe.component_code_field].value + '_' + top.bugframe.document.bugform.elements[top.bugframe.subcomp_code_field].value + '.html';
    return (problem_type_list + suffix);
}
...
//KKOUSHIK Enh 35781150 - BugDB header fields to be made mandatory on product hierarchy level
...
bug_comments_help                  = server_help_path + 'bug_comments_help_dq.html';  //added for ER#33273525 on 25-AUG-2021

current_list = -1;
problem_type_list                = server_lists_path + 'problem_type_list_';  // completed at run-time
problem_type_list_uid = 31;
...
var xmlDwnldHttp = null;

function startDownload()
{
    if (xmlDwnldHttp.readyState == 4) {        
        var xmlDoc = xmlDwnldHttp.responseXML.documentElement;
        var resp_code = xmlDoc.getElementsByTagName("BUG_RESP_CODE")[0].childNodes[0].nodeValue;
        var resp_msg = xmlDoc.getElementsByTagName("BUG_RESP_MSG")[0].childNodes[0].nodeValue;
        
        if (resp_code != 200)
            alert (resp_msg);
        else
            window.open(resp_msg);
    }
}

function getDownloadUrl(rptno, filename)
{    
    xmlDwnldHttp = top.GetXmlHttpObject();
    if (xmlDwnldHttp == null)
    {
       alert ("Your browser does not support AJAX!");
       return;
    }
    var url = "BUGFILE_ATTACHMENTS.PRINT_PAR_URL?pn_rptno="+rptno+"&pv_file_name="+encodeURIComponent(filename)+"&pv_origin=UI";
//alert(url);
    xmlDwnldHttp.onreadystatechange = startDownload;
    xmlDwnldHttp.open("GET",url,true);
    xmlDwnldHttp.send(null);
}

// Function to get the active session to clone apex session
function get_apex_last_session_id(pUserEmail)
   {  
      var v_user_agent = encodeURI(navigator.userAgent);
      var v_result;
      $.ajax({
                        url         : '/pls/bug/last_apex_user_session',
                        type        : 'GET',
                        crossDomain : true,
                        datatype    : 'text',
                        data        : 'P_BUG_USER_EMAIL='+pUserEmail+'&P_USER_AGENT='+v_user_agent,
...
//function to resize bug comments iFrame added by KKOUSHIK ER#34773107
function resizeBugCommentsFrame(pHeight) {
top.bugframe.document.querySelector('#BCIFRAME').style.height = pHeight;
}
